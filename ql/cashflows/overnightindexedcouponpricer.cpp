...
namespace QuantLib {
    ...
    Rate CompoundingOvernightIndexedCouponPricer::swapletRate() const {
        Rate swapletRate, effectiveSpread, effectiveIndexFixing;
        std::tie(swapletRate, effectiveSpread, effectiveIndexFixing) = compute(coupon_->accrualEndDate());
    ...
    Rate CompoundingOvernightIndexedCouponPricer::averageRate(const Date& date) const {
        Rate rate, effectiveSpread, effectiveIndexFixing;
        std::tie(rate, effectiveSpread, effectiveIndexFixing) = compute(date);
        return rate;
    }

    Rate CompoundingOvernightIndexedCouponPricer::effectiveSpread() const {
        Rate r, effectiveSpread, effectiveIndexFixing;
        std::tie(r, effectiveSpread, effectiveIndexFixing) = compute(coupon_->accrualEndDate());
        effectiveSpread_ = effectiveSpread;
    ...
    std::tuple<Rate, Spread, Rate> CompoundingOvernightIndexedCouponPricer::compute(const Date& date) const {
        ...
        while (i < n && fixingDates[i] < today) {
            ...
            const std::pair<Real, Real> gfPair = growthFactor(fixing, i);
            compoundFactorWithoutSpread *= gfPair.first;
            compoundFactor *= gfPair.second;
            ++i;
        }
            ...
            if (coupon_->canApplyTelescopicFormula()) {
                ...
                if (telescopicStartIdx < telescopicEndIdx) {
                    while (i < telescopicStartIdx) {
                        ...
                        const std::pair<Real, Real> gfPair = projGrowthFactor(i);
                        compoundFactorWithoutSpread *= gfPair.first;
                        compoundFactor *= gfPair.second;
                        ++i;
                    }
        ...
        const Date rateAccrualStartDate = applyObservationShift ? valueDates.front() : interestDates.front();
        const Date rateAccrualEndDate = applyObservationShift ? valueDates[n]
                                                              : std::min(date, interestDates[n]);
